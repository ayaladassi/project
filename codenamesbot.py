...
model = gensim.models.KeyedVectors.load_word2vec_format(
    'C:/Users/212318026/PycharmProjects/project/GoogleNews-vectors-negative300.bin', binary=True, limit=200000
)
result = model.most_similar(positive=['queen', 'man'], negative=['woman'], topn=1)
print(result)


with open("C:/Users/212318026/PycharmProjects/project/words.txt",encoding="utf8") as f:
    words = f.readlines()
	...
	def __init__(self):
		...
		def agent_begin():
			...
			def button_action(word):
				...
				if word in red and curr_player == 0 and num_guesses > 0:
					...
					red.remove(word)
					num_guesses -= 1
					bwc.get(word).configure(background = '#b32400', highlightthickness = 50)
					...
					blue.remove(word)
					num_guesses -= 1
					bwc.get(word).configure(background = '#0066cc', highlightthickness = 50)
					...
					bwc.get(word).configure(background = 'black', highlightthickness = 50)
					victory(0)
				else: #neutral case
					neutral.remove(word)
					if curr_player == 0:
						...
						current_hint.configure(text = "Clue is " + next_clue[0])
					curr_player = 1 - curr_player
					bwc.get(word).configure(background = '#bfbfbf', highlightthickness = 50)
				if red_score == 9:
					victory(red_score)
			...
			for i in range(5):
				for j in range(5):
					s = board[i][j]
					game_button = tk.Button(game_state, text = board[i][j], background = 'white', highlightthickness = 50, font = ('Courier New', 10), command = lambda v=s: button_action(v))
		...
		def spy_begin():
			...
			def computer_guess():
				...
				if clue not in model.key_to_index:
				...
				for g in guess_list:
					if g in red:
						bwc.get(g).configure(background = 'black', highlightthickness = 50)
						red_score += 1
						red_team.configure(text = "Red: " + str(red_score))
						num_guesses -= 1
						if red_score == 9:
			...
			for i in range(5):
				for j in range(5):
					if board[i][j] in red:
						game_button = tk.Button(game_state, text = board[i][j], background = '#b32400', font = ('Courier New', 10), highlightthickness=50)
					elif board[i][j] in blue:
						game_button = tk.Button(game_state, text = board[i][j], background = '#0066cc', font = ('Courier New', 10), highlightthickness=50)
					elif board[i][j] in neutral:
						game_button = tk.Button(game_state, text = board[i][j], background = '#bfbfbf', font = ('Courier New', 10), highlightthickness=50)
					else:
						game_button = tk.Button(game_state, text = board[i][j], background = '#404040', font = ('Courier New', 10), highlightthickness=50)
					game_button.place(relx = i/5, rely = j/5, relwidth = 0.2, relheight = 0.2)
					bwc[board[i][j]] = game_button
		...
		startpage.pack()
		background_image = tk.PhotoImage(file = 'C:/Users/212318026/PycharmProjects/codeNames/codenames-master/spymaster.png')
		...
		label = tk.Label(frame, text = "Codenames", font = ('Courier', 24), bg = 'white')
